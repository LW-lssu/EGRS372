...
double target_forward=0.0;        	// DISTANCE THE ROBOT IS TRYING TO MOVE
...
int side_count=0;   	   	// Number of sides user determines for shape
float side_length=0.0;   	   	// Length of each side in shape

double last_tf_x = INITIALIZE_VALUE;
double last_tf_y = INITIALIZE_VALUE;
double circle_dist_traveled = 0.0;
...
void Turnprog(const tf::tfMessage cvalue)
{
 ...
 // ======================================
// TF CIRCLE DISTANCE CALLBACK
// =======================================
void tfCircleCallback(const tf::tfMessage& msg)
{
  double x = msg.transforms[0].transform.translation.x;
  double y = msg.transforms[0].transform.translation.y;

  if(last_tf_x == INITIALIZE_VALUE)
  {
    last_tf_x = x;
    last_tf_y = y;
    return;
  ...
  double dx = x - last_tf_x;
  double dy = y - last_tf_y;

  circle_dist_traveled += sqrt(dx*dx + dy*dy);
...
int main(int argc, char **argv)
{
  ...
  fflush(stdin);
  std::cout << "Enter the number of sides for shape(10> will be a circle): ";
  std::cin >> side_count;
  std::cout << "Enter the length of each side(0.1 to 1.0): ";
  std::cin >> side_length;

  target_forward = side_length;
  ...
  if(side_count > 10)
  {
    circle_dist_traveled = 0.0;
    last_tf_x = INITIALIZE_VALUE;
    last_tf_y = INITIALIZE_VALUE;

    double radius = side_length;
    double circle_dist = radius * 2 * PI;

    double v = 0.2;
    double w = -v / radius;

    if(radius < 0.2)				// 0.1 good
    {
      circle_dist = circle_dist * 0.82;
    }
    if((radius < 0.3) && (radius >= 0.2))	// 0.2 good
    {
      circle_dist = circle_dist * 1.16;
    }
    if((radius < 0.4) && (radius >= 0.3))	// 0.3 good
    {
      circle_dist = circle_dist * 1.21;
    }
    if((radius < 0.5) && (radius >= 0.4))	// 0.4 good
    {
      circle_dist = circle_dist * 1.18;
    }
    if((radius < 0.6) && (radius >= 0.5))	// 0.5
    {
      circle_dist = circle_dist * 1.10;
    }
     if((radius < 0.7) && (radius >= 0.6))	// 0.6
    {
      circle_dist = circle_dist * 1.12;
    }
     if((radius < 0.8) && (radius >= 0.7))	// 0.7
    {
      circle_dist = circle_dist * 1.00;
    }
     if((radius < 0.9) && (radius >= 0.8))	// 0.8
    {
      circle_dist = circle_dist * 1.00;
    }
     if((radius < 1.0) && (radius >= 0.9))	// 0.9
    {
      circle_dist = circle_dist * 1.0;
    }
     if(radius >= 1.0)				// 1.0
    {
      circle_dist = circle_dist * 0.9;
    }

    tf = n.subscribe("/tf", 1, tfCircleCallback);

    while(ros::ok() && circle_dist_traveled < circle_dist)
    {
      c.linear.x = v;
      c.angular.z = w;
      vel_pub.publish(c);
      ...
      ros::spinOnce();
      loop_rate.sleep();
     //std::cout << circle_dist_traveled << std::endl;
    }

    tf.shutdown();
    std::cout << "Target Distance: " << circle_dist << std::endl;
  }
  ...
  for(int i=0;i<side_count;i++)
  {
	...
	target_angle=target_angle- 2*PI/ side_count;
   ...
   //PUBLISHES 0 VELOCITY TO STOP THE ROBOT WITHOUT HAVING TO CTAL C
  	c.linear.x=0.0;
  	c.angular.z=0.0;
	for(int j = 0; j < 5; j++)
	{
